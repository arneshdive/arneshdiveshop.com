...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  const canonical = `${siteConfig.url}${getPathname({ href: `/blog/${slug}`, locale })}`;
  const languages: Record<string, string> = {};
  for (const loc of routing.locales) {
    languages[loc] = `${siteConfig.url}${getPathname({ href: `/blog/${slug}`, locale: loc })}`;
  }
  languages['x-default'] = `${siteConfig.url}/blog/${slug}`;
      ...
      <JsonLd
        data={{
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: t('title'), item: `${siteConfig.url}/blog` },
        ...
        <header className="bg-neutral-50">
          <div className="mx-auto max-w-[1240px] px-6 pb-12 pt-6 lg:px-12 lg:pb-16 lg:pt-10">
            <nav className="mb-14 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.13em] text-neutral-500 lg:mb-20">
              <Link href="/" className="hover:text-neutral-900">{t('breadcrumb.home')}</Link>
              <span aria-hidden="true">/</span>
              <Link href="/blog" className="hover:text-neutral-900">{t('breadcrumb.journal')}</Link>
              <span aria-hidden="true">/</span>
        ...
        <div className="mx-auto grid max-w-[1120px] gap-12 px-6 py-16 lg:grid-cols-[220px_minmax(0,680px)] lg:gap-20 lg:px-12 lg:py-24">
          <aside className="hidden lg:block">
            <div className="sticky top-28 border-t border-neutral-900 pt-5">
              <p className="mb-5 text-[10px] font-semibold uppercase tracking-[0.22em] text-neutral-500">{t('inThisArticle')}</p>
          ...
          <div>
            ...
            {post.ctaHref && post.ctaLabel && (
              <div className="mt-20 border-y border-neutral-300 py-8">
                <p className="mb-4 text-xs uppercase tracking-[0.16em] text-neutral-500">{t('continueFromHere')}</p>
        ...
        <section className="border-t border-neutral-200 py-20 lg:py-28">
          <div className="mx-auto max-w-[1440px] px-6 lg:px-12">
            <div className="mb-10 flex items-end justify-between gap-6">
              <div>
                <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.25em] text-neutral-500">{t('continueReading')}</p>
                <h2 className="text-3xl font-bold tracking-tighter lg:text-[44px]">{t('relatedArticles')}</h2>
              </div>
